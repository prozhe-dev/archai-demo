...
def generate_all_files(imgpath, info, position=None, rotation=None, CubiCasa=False, SR=[2,"lapsrn"]):
    ...
    if CubiCasa == True:
        ...
        # Generate icons (including windows and doors) from CubiCasa predictions
        new_shape = generate_icons_file(imgpath, info, polygons=polygons, types=types)
        shape = validate_shape(shape, new_shape)
    ...
    new_shape = generate_icons_file(imgpath, info)
    shape = validate_shape(shape, new_shape)
...
def generate_icons_file(img_path, info, polygons=[], types = []):
    ...
    # Specifically for doors and windows
...
def generate_walls_file(img_path, info, CubiCasa=False, polygons=[],types=[]):
    ...
    if CubiCasa == True:
        ...
        Walls = unary_union([Polygon(j) for i,j in enumerate(polygons) if types[i]['type']=='wall'])
        
        Icons = unary_union([Polygon(j) for i,j in enumerate(polygons) if types[i]['type']=='icon' and types[i]['class'] in [1,2]])
