...
def detectOuterContours2(detect_img, output_img = None, color = [255, 255, 255]):
    """
    ...
    epsilon = 0.001*cv2.arcLength(largest_contour,True)
    # epsilon = (0.03- 0.00000001)*cv2.arcLength(largest_contour,True)
...
def is_perpendicular(p1, p2, p3):
    ...
    cos = dot / norms
    # Allow some tolerance around 90 degrees (0 cos)
    return abs(cos) < 0.1  # Adjust tolerance as needed
def detectOuterContours(detect_img, output_img = None, color = [255, 255, 255]):
    ret, thresh = cv2.threshold(detect_img, 230, 255, cv2.THRESH_BINARY_INV)

    contours, hierarchy = cv2.findContours(thresh.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    print("Raw contour points:", contours)
    largest_contour_area = 0
    for cnt in contours:
        if (cv2.contourArea(cnt) > largest_contour_area):
            largest_contour_area = cv2.contourArea(cnt)
    ...
    # First get a rough approximation
    epsilon = 0.001*cv2.arcLength(largest_contour,True)
    approx = cv2.approxPolyDP(largest_contour,epsilon,True)
    print("Approximated points:", approx)

    # Function to check if angle is approximately 90 degrees


    # Convert contour to numpy array of points
    points = approx.reshape(-1, 2)
    
    # Keep points that form perpendicular angles
    filtered_points = []
    n = len(points)
    for i in range(n):
        ...
        # If points form perpendicular angle or distance is significant, keep the point
        if is_perpendicular(prev_point, curr_point, next_point):
            filtered_points.append(curr_point)
    
    filtered_points = np.array(filtered_points).reshape(-1, 1, 2)
    
    if output_img is not None:
        final = cv2.drawContours(output_img, [filtered_points], 0, color)
        
    return filtered_points, output_img
